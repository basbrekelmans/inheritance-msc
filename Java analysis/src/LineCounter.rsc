...
import IO;					//readFileLines
import util::Resources;		//getProject
import FileInfo;            //defaultOutputPath
...
public void writeLinesOfCode(loc projectLoc) {

	int codeLines = 0;
	int commentLines = 0;
	int blankLines = 0;
	visit (getProject(projectLoc)) {
		case \file(loc fileLoc): {
			<fCode, fComment, fBlank> = countLines(fileLoc);
			codeLines += fCode;
	...
	fileLoc += projectLoc.authority + "-loc.csv";
	writeFile(fileLoc, "LinesOfCode;LinesOfComment;BlankLines\r\n<codeLines>;<commentLines>;<blankLines>");	
